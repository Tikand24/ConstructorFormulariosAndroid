...
public class CancerMamaActivity extends AppCompatActivity implements View.OnClickListener{

    LinearLayout contenedorCancer;
    ArrayList<CoreGuardado> paraPostFormato;
    String persona;
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_cancer_mama);

        contenedorCancer = (LinearLayout) findViewById(R.id.contenedorCancerMama);
        ...
        for (int i=0; i<bloquesFormato.size();i++){
            ...
            contenedorCancer.addView(bloque);
            ...
            for (int j=0; j<bloquesFormato.get(i).getPreguntas().size(); j++){
                ...
                if (bloquesFormato.get(i).getPreguntas().get(j).getTipo().equals("desplegable")){
                     ...
                     */
                    contenedorCancer.addView(pregunta);
                    contenedorCancer.addView(select);
                }
                ...
                if (bloquesFormato.get(i).getPreguntas().get(j).getTipo().equals("multiple seleccion")){
                    ...
                    for (int k=0; k<bloquesFormato.get(i).getPreguntas().get(j).getRespuestas().size();k++){
                        ...
                        contenedorCancer.addView(check);
                    }
                ...
                if (bloquesFormato.get(i).getPreguntas().get(j).getTipo().equals("fecha")){
                        ...
                        public void onFocusChange(View v, boolean b) {
                            ...
                            if (v.getId()==fecha.getId()){
                                if (b){
                                    ...
                                    DatePickerDialog d = new DatePickerDialog(CancerMamaActivity.this,dpd,c.get(Calendar.YEAR),c.get(Calendar.MONTH),c.get(Calendar.DAY_OF_MONTH));
                                    d.show();
                                }
                            }
                        }
                    ...
                    contenedorCancer.addView(pregunta);
                    contenedorCancer.addView(fecha);
                }
