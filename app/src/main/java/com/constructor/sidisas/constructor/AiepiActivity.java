...
public class AiepiActivity extends AppCompatActivity implements View.OnClickListener{

    LinearLayout contenedorAiepi;
    ArrayList<CoreGuardado> paraPostAiepi;
    String persona;
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_aiepi);
        contenedorAiepi = (LinearLayout) findViewById(R.id.contenedorAiepi);
        ConexionSQLiteHelper conn = new ConexionSQLiteHelper(this,"newaps",null,1);
        String[] formatoAiepi={"3"};
        ArrayList<Bloque> bloquesAiepi=null;
        CoreConstructor constructorFormato=new CoreConstructor();
        bloquesAiepi=constructorFormato.getMainCore(formatoAiepi,conn.getWritableDatabase());
        conn.close();

        paraPostAiepi = new ArrayList<CoreGuardado>();
        for (int i=0; i<bloquesAiepi.size();i++){
            /*
            SECCION DEL BLOQUE
            ...
            bloque.setText(bloquesAiepi.get(i).getNombre());
            ...
            contenedorAiepi.addView(bloque);


            //Se recorren las preguntas del bloque
            for (int j=0; j<bloquesAiepi.get(i).getPreguntas().size(); j++){
                CoreGuardado paraGuardar=new CoreGuardado();
                paraGuardar.setTipoPregunta(bloquesAiepi.get(i).getPreguntas().get(j).getTipo());
                paraGuardar.setRespuestas(bloquesAiepi.get(i).getPreguntas().get(j).getRespuestas());
                paraGuardar.setPregunta(bloquesAiepi.get(i).getPreguntas().get(j).getNombre());
                if (bloquesAiepi.get(i).getPreguntas().get(j).getValidacion().equals("obligatorio")){
                ...
                if (bloquesAiepi.get(i).getPreguntas().get(j).getTipo().equals("desplegable")){
                    ...
                    pregunta.setText(bloquesAiepi.get(i).getPreguntas().get(j).getNombre());
                    ...
                    for (int k=0; k<bloquesAiepi.get(i).getPreguntas().get(j).getRespuestas().size();k++){
                        respuestasSelect.add(bloquesAiepi.get(i).getPreguntas().get(j).getRespuestas().get(k).getNombre());
                    }
                     ...
                     */
                    contenedorAiepi.addView(pregunta);
                    contenedorAiepi.addView(select);
                }

                //CREACION DE PREGUNTAS DE TIPO RESPUESTA CORTA O PARRAFO
                if(bloquesAiepi.get(i).getPreguntas().get(j).getTipo().equals("respuesta corta") ||
                        bloquesAiepi.get(i).getPreguntas().get(j).getTipo().equals("parrafo")){
                    ...
                    pregunta.setHint(bloquesAiepi.get(i).getPreguntas().get(j).getNombre());
                    LinearLayout.LayoutParams layoutParamPregunta = new LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.WRAP_CONTENT);
                    layoutParamPregunta.setMargins(0,30,0,15);
                    pregunta.setLayoutParams(layoutParamPregunta);
                    /*
                ...
                if (bloquesAiepi.get(i).getPreguntas().get(j).getTipo().equals("multiple seleccion")){
                    ...
                    for (int k=0; k<bloquesAiepi.get(i).getPreguntas().get(j).getRespuestas().size();k++){
                        ...
                        check.setText(bloquesAiepi.get(i).getPreguntas().get(j).getRespuestas().get(k).getNombre());
                        check.setTextColor(Color.BLACK);
                        contenedorAiepi.addView(check);
                    }
                    paraGuardar.setIdElemento(listaIdCheck);
                }
                if (bloquesAiepi.get(i).getPreguntas().get(j).getTipo().equals("fecha")){
                    TextView pregunta = new TextView(getApplicationContext());
                    pregunta.setText(bloquesAiepi.get(i).getPreguntas().get(j).getNombre());
                    pregunta.setBackgroundColor(Color.WHITE);
                    pregunta.setTextColor(Color.BLACK);
                        ...
                        public void onFocusChange(View v, boolean b) {
                            ...
                            if (v.getId()==fecha.getId()){
                                if (b){
                                    ...
                                    DatePickerDialog d = new DatePickerDialog(AiepiActivity.this,dpd,c.get(Calendar.YEAR),c.get(Calendar.MONTH),c.get(Calendar.DAY_OF_MONTH));
                                    d.show();
                                }
                            }
                        }
                    ...
                    contenedorAiepi.addView(pregunta);
                    contenedorAiepi.addView(fecha);
                }

                paraPostAiepi.add(paraGuardar);
            }
        }
    }

    public void limpiarParaPost(){
        for (int i=0; i<paraPostAiepi.size();i++){
            paraPostAiepi.get(i).setIdRespuesta(new ArrayList<Respuesta>());
            paraPostAiepi.get(i).setDescripcion("");
        }
    }
    ...
    public void onClick(View v) {
        switch (v.getId()){
            case R.id.buttonEnviarDatosAiepi:
                ...
                for (int i=0; i<paraPostAiepi.size();i++){
                    if (paraPostAiepi.get(i).getTipoPregunta().equals("desplegable")){
                        Spinner spinner= (Spinner) findViewById(paraPostAiepi.get(i).getIdElemento().get(0));
                        if (paraPostAiepi.get(i).getRequerido()){
                            if (spinner.getSelectedItemPosition()==0){
                                Toast.makeText(getApplicationContext(), "Debe seleccionar una respuesta a la pregunta:"+paraPostAiepi.get(i).getPregunta(), Toast.LENGTH_LONG).show();
                                limpiarParaPost();
                                return;
                            }
                        }
                        ArrayList<Respuesta> arrayRespuesta=new ArrayList<Respuesta>();
                        arrayRespuesta.add(paraPostAiepi.get(i).getRespuestas().get((spinner.getSelectedItemPosition()-1)));
                        paraPostAiepi.get(i).setIdRespuesta(arrayRespuesta);
                    }
                    if (paraPostAiepi.get(i).getTipoPregunta().equals("respuesta corta") ||
                            paraPostAiepi.get(i).getTipoPregunta().equals("parrafo")){
                        TextInputEditText text= (TextInputEditText) findViewById(paraPostAiepi.get(i).getIdElemento().get(0));
                        if (paraPostAiepi.get(i).getRequerido()){
                            if (text.getText().length()==0){
                                Toast.makeText(getApplicationContext(), "Debe digitar una respuesta a la pregunta:"+paraPostAiepi.get(i).getPregunta(), Toast.LENGTH_LONG).show();
                                limpiarParaPost();
                                return;
                            }
                        }
                        ArrayList<Respuesta> arrayRespuesta=new ArrayList<Respuesta>();
                        arrayRespuesta.add(paraPostAiepi.get(i).getRespuestas().get(0));
                        paraPostAiepi.get(i).setIdRespuesta(arrayRespuesta);
                        paraPostAiepi.get(i).setDescripcion(text.getText().toString());
                    }
                    if (paraPostAiepi.get(i).getTipoPregunta().equals("multiple seleccion")){
                        int cuantosCheckeados=0;
                        ArrayList<Respuesta> arrayRespuesta=new ArrayList<Respuesta>();
                        for (int j=0; j<paraPostAiepi.get(i).getIdElemento().size(); j++){
                            CheckBox check= (CheckBox) findViewById(paraPostAiepi.get(i).getIdElemento().get(j));
                            if (check.isChecked()){
                                arrayRespuesta.add(paraPostAiepi.get(i).getRespuestas().get(j));
                                cuantosCheckeados++;
                            }
                        }
                        if (paraPostAiepi.get(i).getRequerido()){
                            if (cuantosCheckeados==0){
                                Toast.makeText(getApplicationContext(), "Debe seleccionar al menos 1 respuesta a la pregunta:"+paraPostAiepi.get(i).getPregunta(), Toast.LENGTH_LONG).show();
                                limpiarParaPost();
                                return;
                            }
                        }
                    ...
                    if (paraPostAiepi.get(i).getTipoPregunta().equals("fecha")){
                        EditText text= (EditText) findViewById(paraPostAiepi.get(i).getIdElemento().get(0));
                        if (paraPostAiepi.get(i).getRequerido()){
                            if (text.getText().length()==0){
                                Toast.makeText(getApplicationContext(), "Debe seleccionar una fecha a la pregunta:"+paraPostAiepi.get(i).getPregunta(), Toast.LENGTH_LONG).show();
                                limpiarParaPost();
                                return;
                            }
                        }
                    ...
                    for (int u=0; i<paraPostAiepi.size(); u++){
                        for (int j=0; j<paraPostAiepi.get(u).getIdRespuesta().size(); j++){
                            ...
                            respuestaPersona.put(Utilidades.RESPUESTAS_PERSONA_RESPUESTA,paraPostAiepi.get(u).getIdRespuesta().get(j).getId());
                            respuestaPersona.put(Utilidades.RESPUESTAS_PERSONA_DESCRIPCION,paraPostAiepi.get(u).getDescripcion());
                            ...
                            switch (paraPostAiepi.get(u).getIdRespuesta().get(j).getFormatoRedireccion()){
                                case "3":
                                    formatoRepetido3++;
                                    break;
                                case "4":
